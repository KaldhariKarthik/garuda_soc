...
module tb_trap_ctrl;
  ...
  reg tkcond,wkcond,mtip_pend,mstat_mie; reg [11:0] irqid; reg [7:0] irqlvl; reg [31:0] vectgt;
  ...
  trap_ctrl dut(.clk_i(clk),.rst_n_i(rst_n),
    ...
    .mti_pending_i(mtip_pend),
    .mtvec_i(mtvec),.mepc_i(mepc),.mstatus_mie_i(mstat_mie),
  ...
  initial begin
    ...
    // MTI taken: cause 0x80000007, non-vectored (mtvec base), tval=0, no CLIC ack
    clr; mtip_pend=1; mstat_mie=1; #1;
      c("mti_is_int",is_int,1); c32("mti_cause",tcause,32'h8000_0007);
      c32("mti_target",trtgt,32'h8000_0000); c32("mti_tval",ttval,0);
      c("mti_no_clic_ack",tk_ack,0); c32("mti_level",clvl,8'd0); c("mti_squash_exmem",sq_exm,1);
    // mstatus.MIE gates taking the MTI
    clr; mtip_pend=1; mstat_mie=0; #1; c("mti_masked",trap_en,0);
    // CLIC external outranks the timer (MEI > MTI)
    clr; mtip_pend=1; mstat_mie=1; tkcond=1; irqid=12'h7; vectgt=32'h9000_0000; #1;
      c("clic_over_mti_ack",tk_ack,1); c32("clic_over_mti_cause",tcause,{1'b1,19'd0,12'h7});
    // an exception outranks the timer
    clr; mtip_pend=1; mstat_mie=1; idill=1; #1;
      c("exc_beats_mti",is_int,0); c32("exc_beats_mti_cause",tcause,32'd2);
    // WFI wakes on a pending timer even with MIE clear (wake is MIE-independent)
    clr; idsys=1; idinstr=32'h10500073; @(posedge clk); #1;
      c("wfi_holds",wfih,1);
      mtip_pend=1; mstat_mie=0; #1; c("wfi_wakes_on_mti",wfih,0);
    clr; #1;
